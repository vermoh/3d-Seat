...
// Палитра выровнена с техдоком: монохром + один акцент для категорий
const PRICE_CATEGORIES = {
  vip:      { id: 'vip',      label: 'VIP',      price: 250, color: '#1a1a1a' },
  standard: { id: 'standard', label: 'Партер',   price: 120, color: '#6a6a6a' },
  economy:  { id: 'economy',  label: 'Балкон',   price:  60, color: '#a8a8a8' },
};
...
const COLORS = {
  ...
  textMuted: '#6a6a6a',
  rule: '#e0e0e0',
  selected: '#b54a2c', // единственный цветовой акцент — выбранное место
  sold: '#d8d8d8',
};
...
function buildVenue() {
  ...
  // Партер
  const parterRows = 12;
  const parterSeatsPerRow = 24;
  const parterSeats = [];
  for (let r = 0; r < parterRows; r++) {
    for (let s = 0; s < parterSeatsPerRow; s++) {
      const isCenter = s >= 8 && s < 16;
      const isFront = r < 3;
      let category = 'standard';
      if (isFront && isCenter) category = 'vip';

      const inAisle = s === 11 || s === 12;
      if (inAisle && r < 10) continue;
      ...
      parterSeats.push({
        ...
        number: s + 1,
        category,
        x: (s - parterSeatsPerRow / 2 + 0.5) * 0.55,
        z: r * 0.85,
  ...
  // Балкон левый
  const balconyRows = 5;
  const balconySeats = 10;
  const leftBalcony = [];
  for (let r = 0; r < balconyRows; r++) {
    for (let s = 0; s < balconySeats; s++) {
      leftBalcony.push({
        id: `bal-l-r${r}-s${s}`,
        row: r + 1,
        number: s + 1,
        category: 'economy',
        x: (s - balconySeats / 2 + 0.5) * 0.55,
        z: r * 0.85,
  ...
  // Балкон правый
  const rightBalcony = leftBalcony.map((s) => ({
    ...s,
    id: s.id.replace('bal-l', 'bal-r'),
    status: Math.random() < 0.2 ? 'sold' : 'available',
  }));
  sections.push({
    id: 'balcony-right',
  ...
  return {
    id: 'venue-mock-001',
    name: 'Концертный зал · DEMO',
    // modelSource: { type: 'file' | 'url', data: ArrayBuffer | string, name: string } | null
    // Задаётся через UI. null → используется программная геометрия.
    modelSource: null,
    modelScale: 1.0,
...
function useSeatSelection(venue) {
  ...
  const total = useMemo(
    () => selectedSeats.reduce((sum, s) => sum + PRICE_CATEGORIES[s.category].price, 0),
    [selectedSeats]
  );
...
function SeatMap2D({ venue, selection }) {
  ...
  const sectionElements = venue.sections.map((section) => {
    ...
    const seats = section.seats.map((seat) => {
      ...
      const cat = PRICE_CATEGORIES[seat.category];
...
function SeatMap3D({ venue, selection, onModelStatusChange }) {
  ...
  useEffect(() => {
    ...
    if (venue.modelSource) {
      ...
      const onSuccess = (gltf) => {
        ...
        const scale = venue.modelScale ?? 1;
        const offset = venue.modelOffset ?? { x: 0, y: 0, z: 0 };
        model.scale.set(scale, scale, scale);
        model.position.set(offset.x, offset.y, offset.z);
        scene.add(model);
        loadedGltfScene = model;
        // Скрываем программное окружение зала, оставляя только сцену + кресла
...
export default function App() {
  const baseVenue = useMemo(() => buildVenue(), []);
    ...
    <div style={styles.app}>
      ...
      <div style={styles.toolbar}>
        ...
        <div style={styles.legend}>
          {Object.values(PRICE_CATEGORIES).map((cat) => (
      ...
      <div style={styles.canvas}>
        {mode === '2d' ? (
          ...
          <SeatMap3D venue={venue} selection={selection} onModelStatusChange={handleModelStatus} />
        )}
                ...
                <div key={seat.id} style={styles.cartRow}>
                  <div>
                    ...
                    <div style={styles.cartCategoryLabel}>
                      <span style={{ ...styles.legendDot, background: PRICE_CATEGORIES[seat.category].color }} />
                      {PRICE_CATEGORIES[seat.category].label}
                    </div>
                  </div>
                  <div style={styles.cartPrice}>${PRICE_CATEGORIES[seat.category].price}</div>
                </div>
              ))}
            </div>
